...
@dataclass
class BacktestParam(BaseBacktestParam):
...
class DeltaBearishCallBacktestParam(BacktestParam):
    def __init__(self, strategy_type, buy_call_delta: float, sell_call_delta: float, **kwargs):
        super().__init__(**kwargs)
        self.strategy_type = strategy_type
        self.buy_call_delta = self.buy_value = buy_call_delta
        self.sell_call_delta = self.sell_value = sell_call_delta
...
class BullishPutBacktestParam(BacktestParam):
    def __init__(self, strategy_type, put_sell_delta: float, put_buy_delta: float, **kwargs):
        super().__init__(**kwargs)
        self.strategy_type = strategy_type
        self.put_sell_delta = put_sell_delta
        self.put_buy_delta = put_buy_delta

    def __post_init__(self):
        super().__post_init__()


class BacktestParamFactory:

    @staticmethod
    def create_param(data: Dict) -> BacktestParam:
        """根据前端数据生成相应的回测参数"""
        from backtest_params import BacktestParam  # 移动导入到这里
        ...
        if 'call_buy_delta' in sp and 'call_sell_delta' in sp:
            ...
            return DeltaBearishCallBacktestParam(strategy_type=strategy_type,
